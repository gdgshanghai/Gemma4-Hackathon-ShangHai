...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%CORE%\scripts\final_competition_launcher.ps1" -Action Install %CHECK_ONLY%
set "EXIT_CODE=%ERRORLEVEL%"
if not "%EXIT_CODE%"=="0" (
  echo.
  echo Finals dependency installation failed. Review the message above.
  if not defined STUDYPILOT_NO_PAUSE pause
)
exit /b %EXIT_CODE%
