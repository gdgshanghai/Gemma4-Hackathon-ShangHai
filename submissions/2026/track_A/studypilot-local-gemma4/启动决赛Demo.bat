...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%CORE%\scripts\final_competition_launcher.ps1" -Action Start -DemoMode %CHECK_ONLY%
set "EXIT_CODE=%ERRORLEVEL%"
if not "%EXIT_CODE%"=="0" (
  echo.
  echo Finals Demo startup failed. Check LM Studio, dependencies, and ports.
  if not defined STUDYPILOT_NO_PAUSE pause
)
exit /b %EXIT_CODE%
